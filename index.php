...
if(isset($_SESSION['Auth']) && $user['ac_status']==1 && !$pagecount){
    ...
    showPage('header',['page_title'=>'UwU - SignUp']);
    showPage('signup');
}elseif(isset($_GET['login'])){
   
    showPage('header',['page_title'=>'UwU- Login']);
    showPage('login');
}elseif(isset($_GET['forgotpassword'])){
    
    showPage('header',['page_title'=>'UwU- forgot_password']);
    showPage('forgot_password');
}
elseif(isset($_GET['admin_login'])){
    showPage('header', ['page_title' => 'Admin Login']);
    showPage('admin_login');
}
elseif(isset($_GET['admin_dashboard'])){
    if (isset($_SESSION['AdminAuth'])) {
        ...
        header('Location: ?admin_login'); // Redirect to admin login if not authenticated
        exit;
    }
}
elseif(isset($_GET['view_profile'])){
    if (isset($_SESSION['AdminAuth'])) {
        $user_id = intval($_GET['view_profile']);
        $profile = getUser($user_id); // Fetch user data
        if ($profile) {
            showPage('header', ['page_title' => $profile['username'] . "'s Profile"]);
            showPage('view_profile', ['profile' => $profile]);
        } else {
            showPage('header', ['page_title' => 'User Not Found']);
            showPage('user_not_found');
        }
