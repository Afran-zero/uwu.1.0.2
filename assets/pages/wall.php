...
<?php } ?>
   ...
   <div class="container wall-container col-md-10 col-sm-12 col-lg-9 rounded-0 d-flex justify-content-between">
        ...
        <div class="col-md-8 col-sm-12" style="max-width:93vw">
            ...
            showError('post_img');
            if (count($posts) < 1): ?>
                <div class="no-posts-container d-flex justify-content-center align-items-center">
                    <p class="no-posts-message p-2 bg-dark border rounded text-center col-md-6 col-12">
                        Follow Someone or Add a new post
                    </p>
                </div>
            <?php endif;
foreach($posts as $post){
    ...
    <?php } else { ?>
        <!-- Add report option -->
        <a href="assets/php/actions.php?reportpost=<?= $post['id'] ?>" class="btn btn-sm btn-warning"><i class="bi bi-flag"></i> Report</a>
    <?php } ?>
</div>
...
foreach ($comments as $comment) {
    ...
    $like_btn_display = checkCommentLikeStatus($comment['id']) ? 'none' : '';
    $unlike_btn_display = checkCommentLikeStatus($comment['id']) ? '' : 'none';
    ?>
    <div class="d-flex align-items-center p-2">
        ...
        <div class="d-flex flex-column justify-content-start align-items-start">
            ...
            <p style="margin:0px;" class="text-muted" style="font-size:small">(<?=show_time($comment['created_at'])?>)</p>
        </div>
        <div class="ms-auto">
            <i class="bi bi-heart-fill unlike-comment text-danger" style="display:<?=$unlike_btn_display?>" data-comment-id="<?=$comment['id']?>"></i>
            <i class="bi bi-heart like-comment" style="display:<?=$like_btn_display?>" data-comment-id="<?=$comment['id']?>"></i>
        </div>
    </div>
    <?php
}
...
<div class="d-flex justify-content-between">
    ...
    <?php
        ...
        <div class="col-lg-4 col-sm-0 overflow-hidden mt-4 p-sm-0 p-md-3">
            ...
            <div>
                <h6 class="text-muted p-2 text-dark">You Can Follow Them</h6>
                <?php
...
if(count($follow_suggestions)<1){
    echo "<p class='p-2 bg-dark border rounded text-center'>No Suggestions For You</p>";
}
                ?>
