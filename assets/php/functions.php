...
function getUser($user_id) {
    if (empty($user_id)) {
        return null; // Return null if user_id is invalid
    }
...
function getFollowSuggestions(){
    ...
    $current_user = $_SESSION['userdata']['id'];
    $query = "SELECT * FROM users WHERE id!=$current_user ORDER BY username ASC";
    $run = mysqli_query($db,$query);
    return mysqli_fetch_all($run,true);
}
...
function deletePost($post_id) {
    ...
    // Mark notifications as read/invalid
...
function reportPost($post_id) {
    ...
    // Fetch post details to include the poster's ID
    $query = "SELECT user_id AS poster_id, post_img FROM posts WHERE id = $post_id";
    $result = mysqli_query($db, $query);

    if (!$result || mysqli_num_rows($result) == 0) {
        die("Error: Post not found or query failed - " . mysqli_error($db));
    }

    $post = mysqli_fetch_assoc($result);
    $poster_id = $post['poster_id'];
    $post_img = mysqli_real_escape_string($db, $post['post_img']);

    $current_user = $_SESSION['userdata']['id']; // The user reporting the post

    // Insert report details into the post_reports table
    $report_query = "INSERT INTO post_reports (post_id, user_id, poster_id, post_img, reported_at) 
                     VALUES ($post_id, $current_user, $poster_id, '$post_img', NOW())";

    if (!mysqli_query($db, $report_query)) {
        die("Error: Failed to insert report - " . mysqli_error($db));
    }
...
function getAllReports() {
    global $db;
    $query = "SELECT report_id, post_id, user_id AS reporter_id, poster_id, post_img, reported_at 
              FROM post_reports 
...
function checkCommentLikeStatus($comment_id) {
    global $db;
    $current_user = $_SESSION['userdata']['id'];
    $query = "SELECT COUNT(*) as row FROM comment_likes WHERE user_id = $current_user AND comment_id = $comment_id";
    $run = mysqli_query($db, $query);
    return mysqli_fetch_assoc($run)['row'];
}
...
function unlikeComment($comment_id) {
    global $db;
    $current_user = $_SESSION['userdata']['id'];
    $query = "DELETE FROM comment_likes WHERE comment_id = $comment_id AND user_id = $current_user";
    return mysqli_query($db, $query);
}
function countCommentLikes($comment_id) {
    global $db;
    $query = "SELECT COUNT(*) as total FROM comment_likes WHERE comment_id = $comment_id";
    $result = mysqli_query($db, $query);
    return mysqli_fetch_assoc($result)['total'];
}
