...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Hide the dropdown when close button is clicked
...
$(".like-comment").click(function () {
    ...
    $.ajax({
        url: "assets/php/ajax.php?likecomment",
        method: "post",
        dataType: "json",
        ...
        success: function (response) {
            if (response.status) {
                $(button).hide();
                $(button).siblings(".unlike-comment").show();
            } else {
...
// Unlike a comment
$(".unlike-comment").click(function () {
    var comment_id = $(this).data("comment-id");
    var button = this;
    $(button).attr("disabled", true);

    $.ajax({
        url: "assets/php/ajax.php?unlikecomment",
        method: "post",
        dataType: "json",
        data: { comment_id: comment_id },
        success: function (response) {
            if (response.status) {
                $(button).hide();
                $(button).siblings(".like-comment").show();
            } else {
                alert("Something went wrong. Please try again.");
            }
            $(button).attr("disabled", false);
